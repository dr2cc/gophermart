...
func TestHandler_signUp(t *testing.T) {
	...
	for _, test := range testTable {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			// Роутер прогоняет запрос через свои механизмы, вызывает ваш хендлер, тот вызывает сервис (мок!),
			// получает ответ и записывает результат в w.
			r.ServeHTTP(w, req)

			// Проверка (Assert, точнее "проверка утверждения").
			// 1. Проверяем код состояния (Status Code)
			// Мы ожидаем, например, 200 OK или 201 Created, если тест успешный.
			// test.expectedStatusCode уже заранее определен в структуре теста.
			assert.Equal(t, test.expectedStatusCode, w.Code)
			// 2. Проверяем тело ответа (Response Body)
			// w.Body.String() возвращает то, что хендлер записал в ответ.
			// Мы сравниваем это с ожидаемой строкой из нашего тест-кейса.

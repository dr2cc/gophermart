...
-- 15:19
CREATE TABLE orders (
    id BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- уникальный id записи
    -- number
    order_id UUID, -- UUID это специальный тип для заказов(!) ключ для поиска? вместо number(?), с 12:23. Разобраться!
    user_id BIGINT NOT NULL REFERENCES users (id) ON DELETE RESTRICT ON UPDATE CASCADE,
    -- TODO: status  ENUM для status - VARCHAR нет!
    status task_status DEFAULT 'NEW', -- Можно задать значение по умолчанию
